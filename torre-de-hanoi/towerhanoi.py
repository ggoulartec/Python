...
def quantDisco():
    ...
    while (iniciarjogo == False):
        ctrl = input("\nInforme qual a quantidade de disco(s): ")
        for i in range(100):
            if ctrl == i.__str__():
                ndiscos = ctrl
        ...
        if ndiscos != "" and int(ndiscos) >= 3:
            ...
            print(
                "|------>  A quantidade mínima de discos para iniciar o jogo deve ser >= 7 discos.")
...
def inputValue():
    ...
    while saida == "":
        ...
        if movSaida == "" and ctrl != False:
            print("A Torre informada esta errada, por favor informe uma torre válida")
            ctrl = False
        else:
            ...
            saida = input("Informe a Torre de Saída: ")
        ...
        while entrada == "" and (not reset and not novaPartida):
            ...
            if movEntrada == "" and ctrl != False:
                print(
                    "A torre informada esta errada, por favor informe uma torre válida.")
                ctrl = False
            else:
                entrada = input(
                    "Informe a Torre de Entrada " + movSaida + ": ")
                ...
                if entrada == "reset":
                    ...
                    if movSaida == movEntrada:
                        print(
                            "Essa jogada não é válida. Você deve mudar sua jogada.")
                        ctrl = False
                    else:
                        ctrl = True
...
def validaAlocacao(torre, operacao, valorMove):
    ...
    if length == 0 and operacao == 1:
        print("Esta torre esta vazia. Você deve mudar sua jogada.")
        return False
        ...
        if valorMove < torre[length - 1]:
            ...
            print(
                "Os últimos discos que estão na torre é menor que o disco [", valorMove, "]. Você deve mudar sua jogada.")
            return False
    else:
        return True
...
def fimJogo():
    print(
        "PARABÉNS, VOCÊ GANHOU...\nAgora o que você acha de aumentar o desafio.\n")

    if input("Para uma nova partida digite [iniciar]: ") == "iniciar":
        iniciar()
    else:
        print("Que pena, então até a próxima... bye bye\n")
        sys.exit(0)
